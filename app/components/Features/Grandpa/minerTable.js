...
const MinerTable = props => {
  ...
  return (
    <ToolBody
      ...
      icon={AccountBalance}
      header="GrandpaCoins"
      subheader=" - Let's make these old coins move FAST">
        ...
        <Table className={classes.table}>
          ...
          <TableBody>
            ...
            <TableRow className={`${classes.tableStripedRow} ${classes.tableRowHover}`}>
              ...
              <TableCell className={classes.tableCell}>
                <h4>{round ? round.current_miner : 'Loading'}</h4>
              </TableCell>
            </TableRow>
            <TableRow className={classes.tableRowHover}>
              <TableCell className={classes.tableCell}>Usurp Fee</TableCell>
              <TableCell className={classes.tableCell}>
                <h4>{round ? `${(Number(round.current_fee.split(' ')[0]) * 1.25).toFixed(4)} EOS` : 'Loading'}</h4>
              </TableCell>
            </TableRow>
            <TableRow className={`${classes.tableStripedRow} ${classes.tableRowHover}`}>
              <TableCell className={classes.tableCell}>Miner changed on</TableCell>
              <TableCell className={classes.tableCell}>
                {round ? `${new Date(round.updated_on / 1000).toLocaleString()}` : 'Loading'}
              </TableCell>
            </TableRow>
            <TableRow className={`${classes.tableRowHover}`}>
